         ...
         if (txData.attendeeEmail) {
           try {
             ...
             await adminDb.collection("activityLogs").add({
               level: "error",
               category: "payment",
               message: `Card IPN: Failed to send ticket email for ref ${OrderMerchantReference}`,
               metadata: { ref: OrderMerchantReference, gatheringId: txData.gatheringId, attendeeEmail: txData.attendeeEmail, error: String(emailErr) },
               createdAt: admin.firestore.FieldValue.serverTimestamp(),
             });
           }
         }
