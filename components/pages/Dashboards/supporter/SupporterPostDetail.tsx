        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <Link href={`/${post.creatorId}`} className="font-semibold text-gray-900 hover:text-blue-600">
                  {creator?.name || post.creatorId}
                </Link>
                <p className="text-xs text-gray-500">
                  @{post.creatorId} · {post.createdAt?.toDate?.()?.toLocaleDateString()}
                </p>
              </div>
            </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200 mb-4">
                ...
                {pages.length > 1 && (
                  <div className="flex items-center justify-between mb-3">
                    <button onClick={() => setDocumentIndex((i) => Math.max(0, i - 1))} disabled={documentIndex === 0} className="p-1.5 bg-white border rounded-lg disabled:opacity-50"><ChevronLeft size={16} /></button>
                    <span className="text-sm text-gray-500">{documentIndex + 1} of {pages.length}</span>
                    <button onClick={() => setDocumentIndex((i) => Math.min(pages.length - 1, i + 1))} disabled={documentIndex === pages.length - 1} className="p-1.5 bg-white border rounded-lg disabled:opacity-50"><ChevronRight size={16} /></button>
                  </div>
                )}
                <iframe src={`https://docs.google.com/viewer?url=${encodeURIComponent(pages[documentIndex])}&embedded=true`} className="w-full h-[500px] rounded-lg bg-white" title="Document" />
              </div>
            )}
            ...
            <div className="flex items-center gap-4 mt-4">
              <button onClick={handleLike} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition ${liked ? "bg-red-50 text-red-500" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}>
                <Heart size={18} className={liked ? "fill-current" : ""} /> {likeCount}
              </button>
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-gray-100 text-gray-500">
          ...
          <div className="border-t border-gray-100 p-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {replyingTo === comment.id && (
                        <div className="mt-2 flex gap-2">
                          <input value={replyText[comment.id] || ""} onChange={(e) => setReplyText((p) => ({ ...p, [comment.id]: e.target.value }))} placeholder="Write a reply..." className="flex-1 text-xs px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      ...
                      {comment.replies && comment.replies.length > 0 && (
                        <div className="mt-2 ml-3 space-y-2">
                          {comment.replies.map((reply) => (
                            <div key={reply.id} className="flex items-start gap-2 pl-2 border-l-2 border-gray-200">
                              <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-500 shrink-0 overflow-hidden">
            ...
            <div className="mt-4 flex gap-2">
              <input value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder="Write a comment..." className="flex-1 text-sm px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" onKeyDown={(e) => e.key === "Enter" && handleComment()} />
