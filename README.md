...
- **Media enhancements on community pages**: YouTube links in video-type posts now show embedded preview; images are click-to-zoom with full-screen lightbox; videos are playable inline with `controlsList="nodownload"`; documents open in Google Docs viewer via "Read Document" button with modal overlay
