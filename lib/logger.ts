...
export function logInfo(category: LogCategory, message: string, data?: Partial<LogEntry>) {
  return logActivity({ level: "info", category, message, ...data });
}

export function logSuccess(category: LogCategory, message: string, data?: Partial<LogEntry>) {
  return logActivity({ level: "success", category, message, ...data });
}

export function logWarning(category: LogCategory, message: string, data?: Partial<LogEntry>) {
  return logActivity({ level: "warning", category, message, ...data });
}

export function logError(category: LogCategory, message: string, data?: Partial<LogEntry>) {
...
export function getCategoryColor(category: LogCategory): string {
  switch (category) {
    ...
    case "payout": return "text-emerald-600 bg-emerald-50";
    case "support": return "text-orange-600 bg-orange-50";
    case "store": return "text-cyan-600 bg-cyan-50";
    case "giveaway": return "text-pink-600 bg-pink-50";
    case "messaging": return "text-indigo-600 bg-indigo-50";
